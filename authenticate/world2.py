...
for index,datafield in enumerate(datafields_list_fundamental6):
    
    alpha_expression = f'group_rank(({datafield})/fnd6_dxd5, subindustry)'
...
for index in range(346, len(alpha_list)):
    ...
    try:
        ...
        print(index)
        pd.DataFrame({'alpha id': [alpha_id]}).to_csv('alpha_id.csv', index=False, header=False, mode='a')
    except:
